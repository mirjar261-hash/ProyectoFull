...
export default function ReporteVentasCreditoPage() {
  ...
  const cargarVentas = async () => {
    try {
      ...
      const filtradas = data.filter(v => v.estado === 'CREDITO')
      filtradas.sort(
  ...
  const exportarExcel = async () => {
    ...
    XLSX.writeFile(workbook, 'reporte_ventas_credito.xlsx')
  }
  ...
  const exportarPDF = async () => {
    ...
    for (const v of ventas) {
      autoTable(doc, {
        startY: currentY,
        head: [['Folio', 'Cliente', 'Items', 'Estado', 'Subtotal', 'IVA', 'Total', 'Saldo', 'Fecha']],
        body: [[
          v.numdoc,
    ...
    doc.save('reporte_ventas_credito.pdf')
  }
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-orange-600">Ventas a créditos</h1>
          ...
          <TableBody>
            {ventas.map(v => (
              <TableRow key={v.id}>
                ...
                <TableCell className="text-right">${Number(v.saldo_pendiente || 0).toFixed(2)}</TableCell>
      ...
      <div className="text-right font-semibold">Total saldo: ${totalSaldo.toFixed(2)}</div>
