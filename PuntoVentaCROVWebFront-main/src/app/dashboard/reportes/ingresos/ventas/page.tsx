...
export default function ReporteVentasPage() {
  ...
  const cargarVentas = async () => {
    try {
      const res = await axios.get(
        `${apiUrl}/venta?sucursalId=${sucursalIdSession}&fechaInicio=${fechaInicio+"T00:00:00.000Z"}&fechaFin=${fechaFin+"T23:59:59.999"}`,
  ...
  const exportarPDF = async () => {
    ...
    for (const v of ventas) {
      autoTable(doc, {
        startY: currentY,
        head: [['Folio', 'Cliente', 'Items', 'Estado', 'Subtotal', 'IVA', 'Total', 'Fecha']],
        body: [[
          v.numdoc,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-orange-600">Reporte de Ventas</h1>
