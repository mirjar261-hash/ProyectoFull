...
import { calcularKpisSemana } from '../controllers/gerente.controller';
import prisma from '../utils/prisma';

const sendWeeklyKpis = async () => {
  ...
  for (const sucursal of sucursales) {
    const kpis = await calcularKpisSemana(sucursal.id, 0, fecha);

    const html = `
      <div style="font-family: Arial, sans-serif; color: #333;">
        <img src="cid:logo" alt="CROV" style="max-width:150px;" />
        <p>Hola jefe, soy tu gerente CROV.</p>
        <p>Aquí están los KPIs de la semana para ${sucursal.nombre_comercial ?? 'tu sucursal'}:</p>
        <ul>
...
export const scheduleWeeklyKpis = () => {
  const schedule = () => {
    const now = new Date();
    ...
    setTimeout(async () => {
      await sendWeeklyKpis().catch((err) =>
